...
//__lasx_f32cx8_load函数从地址x中加载8个半精度浮点数，并将它们转换为单精度浮点数（fp32），
//然后将它们存储在一个float类型的数组tmp中。最后，该函数将tmp数组作为参数，通过__lasx_xvld指令以单精度浮点数格式加载到一个__m256类型的向量中，并返回该向量。
...
inline static void ggml_vec_scale_f32(const int n, float * y, const float   v) {
    ...
    //在AVX和lasx中，GGML_F32_STEP=32, GGML_F32_EPR=8(8代表8个float，8*32=256)
    //GGML_F32_ARR = GGML_F32_STEP / GGML_F32_EPR
    for (int i = 0; i < np; i += GGML_F32_STEP) {
        for (int j = 0; j < GGML_F32_ARR; j++) {
            ay[j] = GGML_F32_VEC_LOAD(y + i + j*GGML_F32_EPR);//一次load8个float
...
static bool ggml_is_contiguous_n(const struct ggml_tensor * tensor, int n) {
    size_t next_nb = ggml_type_size(tensor->type);//next_nb可能是18
    //tensor->ne[0]可能是4096，block_size可能是32，nb[0]=18
    if (tensor->ne[0] != ggml_blck_size(tensor->type) && tensor->nb[0] != next_nb) {
        return false;
    }
    // next_nb = 18 * 4096 / 32 = 2304
    next_nb *= tensor->ne[0]/ggml_blck_size(tensor->type);
    for (int i = 1; i < GGML_MAX_DIMS; i++) { //GGML_MAX_DIMS=4
        //ne[1]=4096,n=0,nb[1]=2304,next_nb=2304
        //ne[2]=1,n=0.....
        if (tensor->ne[i] != 1) {
            if (i > n) {
...
static void ggml_compute_forward_add_q_f32(
    ...
    //使用src0的dequantize方法，dst的quantize方法
...
static void ggml_compute_forward_mul_mat_one_chunk(
    ...
    //如果src1->type不是src0对应的vec_dot_type的话，从wdata里拿src1量化后的数据
    ...
    float tmp[32];
    //ir1_end = ir0_end = blck_1 = blck_0 = 16
    for (int64_t iir1 = ir1_start; iir1 < ir1_end; iir1 += blck_1) {
        for (int64_t iir0 = ir0_start; iir0 < ir0_end; iir0 += blck_0) {
            for (int64_t ir1 = iir1; ir1 < iir1 + blck_1 && ir1 < ir1_end; ir1 += num_rows_per_vec_dot) {
                ...
                //src0的数据可能是不同的第2和第3维里的ir0_start到ir0_end
...
static void ggml_compute_forward_mul_mat(
    ...
    const int ith = params->ith;//现在是第几个thread
    const int nth = params->nth;//总共有几个thread
        ...
        for (int64_t i13 = 0; i13 < ne13; i13++)
            for (int64_t i12 = 0; i12 < ne12; i12++)
                if (!llamafile_sgemm(ne01, ne11, ne00/ggml_blck_size(src0->type),
                                     ...
                                     src1->type,
                                     dst->type))
                    //这一次使用llamafile_sgemm可能因为src1还没有量化，类型不支持而失败
                    //于是进入 GGML_TASK_TYPE_INIT 对src1进行量化（可能是一个fp32->q8_0的量化）
                    goto UseGgmlGemm1;
        return;
    }
UseGgmlGemm1:;
#endif
    //INIT主要针对src0量化了但是src1还没有量化的情况，需要量化src1到指定格式
    //此处src1不一定是被量化为和src0相同的精度，比如src0为q4_0，src1为fp32，src1应该被量化为q8_0
    if (params->type == GGML_TASK_TYPE_INIT) {
        ...
        if (src1->type != vec_dot_type) {
            ...
            //vec_dot_type为GGML_TYPE_Q8_0，则row_size=
            const size_t row_size = ggml_row_size(vec_dot_type, ne10);
            ...
            for (int64_t i13 = 0; i13 < ne13; ++i13) {
                for (int64_t i12 = 0; i12 < ne12; ++i12) {
                    for (int64_t i11 = 0; i11 < ne11; ++i11) {
                        //from_float_to_vec_dot可能是quantize_row_q8_0, ne10=4096
                        //量化后的数据存储在wdata里，src1的原始数据以及类型不会改变
    ...
    if (src1->type != vec_dot_type) {
        const void* wdata = (src1->type == vec_dot_type) ? src1->data : params->wdata;
        const size_t row_size = ggml_row_size(vec_dot_type, ne10);
        //现在src1已经被量化为需要的格式了，看一下此时src0+src1的格式sgemm是否支持
        //支持的话使用sgemm中的tiny_blas处理；
        //不支持的话还是使用llama.cpp原生的方法
        for (int64_t i13 = 0; i13 < ne13; i13++)
            for (int64_t i12 = 0; i12 < ne12; i12++)
                //nb[0] = ggml_type_size(type); nb[1] = nb[0] * (ne[0] / ggml_blck_size(type)) + padding 
                //src0和src1的第0维是一样的，比如都是4096
                if (!llamafile_sgemm(ne01, ne11, ne00/ggml_blck_size(src0->type),
                                     (const char *)src0->data + i12/r2*nb02 + i13/r3*nb03,
                                     nb01/ggml_type_size(src0->type), // ne00/ggml_blck_size(src0->type)
                                     (const char *)wdata + (i12*ne11 + i13*ne12*ne11)*row_size,
                                     row_size/ggml_type_size(vec_dot_type), // ne10/ggml_blck_size(vec_dot_type)
                                     (char *)dst->data + i12*nb2 + i13*nb3,
                                     nb1/ggml_type_size(dst->type), // ne0/ggml_blck_size(dst->type)
                                     ith, nth,
                                     params->type,
                                     src0->type,
                                     vec_dot_type,
...
UseGgmlGemm2:;
    ...
    // chunk切的是dst，不同的chunk可以由不同的线程处理
    int chunk_size = 16;
    ...
    // dr是实际上chunk的行和列大小，因为可能切块之前列数<16，没法切块
    ...
    //nchunk0=2，nchunk1=4，dr0=16, dr1=16
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor, struct ggml_compute_state * state) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_ADD1:
            {//add1和add的区别是
            // add1是src0的每个元素都和同一个元素相加
            // add是src0的每个元素都和src1的对应元素相加
...
struct ggml_cplan ggml_graph_plan(const struct ggml_cgraph * cgraph, int n_threads) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        //work_size是graph中各个节点中所需的最大size
...
size_t ggml_quantize_chunk(
        enum ggml_type   type, //GGML_TYPE_Q4_0
           const float * src,
                  void * dst,
               int64_t   start, //本次处理的数据从src的第start个元素开始 first_row * n_per_row；如果nthread=1的话，start=0
               int64_t   nrows,  //4->本次处理4行；如果nthread=1的话，nrows即使32000
               int64_t   n_per_row, //4096
           const float * imatrix) {
    const int64_t n = (int64_t) nrows * n_per_row; //本次要处理的元素个数 4*4096; 如果nthread=1的话，n=32000 * 4096
    GGML_ASSERT(n % type_traits[type].blck_size == 0);

    if (ggml_quantize_requires_imatrix(type)) {
        GGML_ASSERT(imatrix != NULL);
    }

    GGML_ASSERT(start % type_traits[type].blck_size == 0); //type_traits[type].blck_size = 32
    GGML_ASSERT(start % n_per_row == 0); //n_per_row = 4096

    ggml_quantize_init(type); // this is noop if already initialized

    const size_t start_row = start / n_per_row; //0
    const size_t row_size  = ggml_row_size(type, n_per_row);     //18 * 4096 / 32 = 2304
