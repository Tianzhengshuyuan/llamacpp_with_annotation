...
struct llm_build_context {
    ...
    struct ggml_cgraph * build_llama() {
        ...
        // batch决定了tensor的第1维
        //一个可能的tensor是ne[0]=4096,ne[1]=99,ne[2]=1,ne[3]=1 
...
static int llama_decode_internal(
    ...
    for (uint32_t cur_token = 0; cur_token < n_tokens_all; cur_token += n_ubatch) {
        ...
        //u_batch可以决定input tensor的第一维，在prompt中第一维可能是99，在decode阶段可能是2
...
static size_t llama_tensor_quantize_internal(enum ggml_type new_type, const float * f32_data, void * new_data, const int64_t chunk_size, int64_t nrows, int64_t n_per_row, const float * imatrix, std::vector<std::thread> & workers, const int nthread) {
    ...
    size_t new_size = 0;
    bool valid = true;
    //定义lambda函数compute，chunk_size=32*512
    auto compute = [&mutex, &counter, &new_size, &valid, new_type, f32_data, new_data, chunk_size,
            nrows, n_per_row, imatrix]() {
        //chunk是在nrows（320000）维度里切的，一个chunk为4
        const int64_t nrows_per_chunk = chunk_size / n_per_row; //32 * 512 / 4096 = 4
        size_t local_size = 0;
        while (true) {
            std::unique_lock<std::mutex> lock(mutex);
            int64_t first_row = counter; counter += nrows_per_chunk;
            //每个线程开始处理之前检查一下，如果超过nrows就退出循环
            if (first_row >= nrows) {
                if (local_size > 0) {
                    new_size += local_size;
                }
            ...
            lock.unlock();
            // this_nrow是计算本次要处理多少行，一般是4，除非剩下的行数不足4
...
static void llama_model_quantize_internal(const std::string & fname_inp, const std::string & fname_out, const llama_model_quantize_params * params) {
    ...
    for (int i = 0; i < ml.n_tensors; ++i) {
        ...
        if (!quantize) {
            ...
            static const int64_t min_chunk_size = 32 * 512;  //16384
            const int64_t chunk_size = n_per_row >= min_chunk_size ? n_per_row : n_per_row * ((min_chunk_size + n_per_row - 1)/n_per_row);

            const int64_t nelements_matrix = tensor->ne[0] * tensor->ne[1];   //前两维相乘
            const int64_t nchunk = (nelements_matrix + chunk_size - 1)/chunk_size;   //chunk的数量4096*32000/(32*512)=8000
            const int64_t nthread_use = nthread > 1 ? std::max((int64_t)1, std::min((int64_t)nthread, nchunk)) : 1;

            // quantize each expert separately since they have different importance matrices
            // 所以第三维是expert数，tensor->ne[2]=1
            new_size = 0;
            // i03代表是第3维的指针
            for (int64_t i03 = 0; i03 < tensor->ne[2]; ++i03) {
                //f32_data是tensor存储地址的指针
